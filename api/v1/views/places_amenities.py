...
def get_all_place_amenities(place_id):
    """ Retrieves all the amenities reviews by place id """
...
def delete_a_place_amenity(place_id=None,
                           amenity_id=None):
...
def link_an_amenity(place_id=None, amenity_id=None):
    ...
    if my_amenity in my_place.amenities:
        return make_response(jsonify
                             (my_amenity.to_dict()), 200)

    my_place.amenities.append(my_amenity)
    storage.save()

    return make_response(jsonify
                         (my_amenity.to_dict()), 201)
